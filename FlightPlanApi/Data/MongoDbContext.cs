...
{
    public class MongoDbContext : IDatabaseAdapter
    {
        private IMongoCollection<BsonDocument> GetCollection(string dabaseName, string collectionName)
        {
            var client = new MongoClient();
            var db = client.GetDatabase(dabaseName);
            var collection = db.GetCollection<BsonDocument>(collectionName);

            return collection;
        }

        private static FlightPlan ConvertBsonToFlightPlan(BsonDocument document)
        {
        ...
        public async Task<List<FlightPlan>> GetAllFlightPlans()
        {
            ...
            List<FlightPlan> flightPlansList = new();
        ...
        public async Task<FlightPlan> GetFlightPlanById(string flightPlanId)
        {
            ...
            return flightPlan ?? (new());
        }

        public async Task<TransactionResult> AddFileFlightPlan(FlightPlan flightPlan)
        {
            var collection = GetCollection("pluralsight", "flight_plans");
